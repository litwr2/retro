...
int terakhor = 640, terakvert = 480;
...
#define TERAK_FLAGS    \
	(SDL_HWSURFACE|SDL_HWACCEL|SDL_ASYNCBLIT|SDL_DOUBLEBUF|SDL_ANYFORMAT|(fullscreen ? SDL_FULLSCREEN : 0))
...
/* Terak screen refresh - single size, no palettes */
extern void scr_refresh_terak(unsigned shift, unsigned full);
...
static SDL_Surface * chargen;

int scr_init_terak() {

    extern unsigned terak_icon[];
    extern unsigned char * compute_icon_mask();
    static char init_done = 0;
    int i;
    ...
    screen = SDL_SetVideoMode(terakhor, terakvert, 0, TERAK_FLAGS);
    if (screen == NULL) {
        fprintf(stderr, _("Couldn't set up video: %s\n"),
                        SDL_GetError());
        exit(1);
    ...
    SDL_WM_SetCaption("Terak 8510/a", "Terak 8510/a");

    chargen = SDL_CreateRGBSurface(SDL_SWSURFACE, terakhor/80, 256*terakvert/24, 32, 0xff0000, 0xff00, 0xff, 0);
	if (!chargen) {
		fprintf(stderr, "Couldn't set up video: %s\n",
			SDL_GetError());
		exit(1);
...
void scr_refresh_terak(unsigned shift, unsigned full) {
	fprintf(stderr, "Refreshing\n");
	/* shift and full so far ignored, everything is blitted */
	int line, col;
	int hscale = terakhor / 640;
	int vscale = terakvert / 240;
	extern d_word vir;
	int scroll = vir >> 8;
	SDL_Rect from, to;
	from.w = to.w = 8 * hscale;
	from.h = to.h = 10 * vscale;
	from.x = 0;
	for (line = 0; line < 25; line++) {
	    to.y = (line - scroll + 25) % 25 * 10 * vscale;
	    if (to.y == 240 * vscale)
		continue;
	    for (col = 0; col < 80; col++) {
		to.x = col * 8 * hscale;
		from.y = terakscr[line][col] * 10 *vscale;
		if (SDL_BlitSurface(chargen, &from, screen, &to))
...
void scr_chargen(int offset, unsigned char val) {
	Uint32 * base = (Uint32*)chargen->pixels;
	int pos = offset / 16;
	int line = offset % 16;
	int hscale = terakhor / 640;
	int vscale = terakvert / 240;
	...
	for (i = 0; i < 8; i++) {
		if (val & (1<<i))
			base[pos*80*hscale*vscale+(line*vscale+v)*8+i*hscale+h] = 0xffffff;
		else
			base[pos*80*hscale*vscale+(line*vscale+v)*8+i*hscale+h] = 0;
	}
	scr_dirty = 1;
}

void scr_putchar(int offset, unsigned char val) {
	int line = offset / 128;
	int col = offset % 128;
	terakscr[line][col] = val;
	scr_dirty = 1;
}
