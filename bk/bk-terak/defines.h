...
extern int boot_read(), boot_write(c_addr, d_word), boot_bwrite(c_addr, d_byte);
extern int scr_init_bk(), scr_init_terak(), scr_write(int, c_addr, d_word), scr_switch(int, int);
...
extern void boot_init(), disk_open(), disk_finish(), tdisk_finish();
extern void io_read_start(), fake_disk_io();
...
extern int terak_init(), terak_read(), terak_write(c_addr, d_word), terak_bwrite(c_addr, d_byte);
extern int service(d_word);
extern void sound_init(), tape_init(), tape_write(unsigned, unsigned);
extern void serial_write(d_word), pagereg_write(d_word), pagereg_bwrite(d_byte);
extern int tape_read();
extern d_word serial_read();
extern void scr_param_change(int, int), ui_load(char*), q_reset(), run(int);
extern int pop(pdp_regs*, d_word*), brx(pdp_regs*, unsigned, unsigned);
extern int push(pdp_regs*, d_word), load_ea(pdp_regs*, d_word*);
extern int load_dst(pdp_regs*, d_word*), loadb_dst(pdp_regs*, d_byte*);
extern int load_src(pdp_regs*, d_word*), loadb_src(pdp_regs*, d_byte*);
extern int store_dst(pdp_regs*, d_word), storeb_dst(pdp_regs*, d_byte);
extern int store_dst_2(pdp_regs*, d_word), storeb_dst_2(pdp_regs*, d_byte);
extern int sl_byte(pdp_regs*, d_word, d_byte);
extern int ll_byte(pdp_regs*, d_word, d_byte*);
extern int sc_word(c_addr, d_word);
extern void showemuhelp(), showbkhelp(), scr_flush(), mouse_event();
extern c_addr disas (c_addr, char*);
extern char *state(pdp_regs*);
extern void timing(pdp_regs*), sound_flush(), scr_sync(), tty_recv();
extern void ev_fire(int), fake_write_file(), fake_tuneup_sequence();
extern void fake_array_with_tuneup(), fake_read_strobe(), fake_sector_io();
extern void init_config(), tty_open(), ev_init(), mem_init(), ui();
extern void plug_mouse(), plug_printer(), plug_covox(), plug_synth();
extern void plug_bkplip();
...
#define TIMER_PRI	0
#define TTY_PRI		1
#define EM_PRI		2	/* Terak console emulator */
#define DISK_PRI	3	/* Terak floppy */
...
#define TTY_FULLSPEED	0371	/* fullspeed toggle */
...
extern void ev_register(unsigned, int (*)(d_word), unsigned long, d_word);
...
/* For BK-0010 */
extern int lc_word(c_addr, d_word*);
