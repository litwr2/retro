...
char * romdir = "/usr/local/share/games/bk"; /* default ROM path */
...
flag_t bflag;		/* block FDD emulation flag */
...
#undef VERBOSE_ILLEGAL
				...
				disas(oldpc, buf);
				fprintf( stderr,
				_("Illegal inst. %s, last branch @ %04x\n"),
					buf, last_branch );
#endif
			...
			default:
				fprintf( stderr, _("\nUnexpected return.\n") );
				fprintf( stderr, _("exec=%d pc=%04x ir=%04x\n"),
					result, oldpc, p->ir );
				flag = 0;
				result2 = OK;
				break;
		...
		if (bkmodel && !fullspeed && ticks >= ticks_timer) {
			scr_sync();
...
run( int flag )
{
	...
	if ( expired != 0.0 )
		...
		speed = 0.0;
	fprintf( stderr, _("Instructions executed: %ld\n"), p->total );
...
char **argv;
{
	...
	if ( args( argc, argv ) < 0 ) {
		...
		fprintf( stderr, _("   -b        Do not emulate disk I/O by block number\n") );
...
by the environment variable BK_PATH.\n"), romdir );
	...
	case 2: /* BK0010.01+FDD */
		rompath10 = monitor10rom;
		rompath12 = 0;
		rompath16 = rompath16 ? rompath16 : diskrom;
		TICK_RATE = 3000000;
		break;
        case 3:	/* BK-0011M */
		rompath16 = rompath16 ? rompath16 : diskrom;
	...
	terak ? scr_init_terak() : scr_init_bk();		/* video display */
	...
	/* Starting frame rate */
	frame_delay = TICK_RATE/refrfreq;
	half_frame_delay = TICK_RATE/refrfreq/2;
