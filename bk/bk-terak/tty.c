...
tty_open()
{
    ...
    special_keys[SDLK_RSUPER] = TTY_AR2;
    ...
    special_keys[SDLK_F10] = TTY_FULLSPEED;
...
#define FN_PICK_WIDGET "bk-loadwidget.tcl"
#define PICKED_FN_FILE "/tmp/bk-loadwidget.fn.tmp"
    FILE *fi;
    if (fork() == 0) {
        remove(PICKED_FN_FILE);
        execlp(FN_PICK_WIDGET, FN_PICK_WIDGET, 0);
        exit(1);
    }
    wait(0);
    if (fi = fopen(PICKED_FN_FILE, "r")) {
        fgets(buf, 256, fi);
        fclose(fi);
        unlink(PICKED_FN_FILE);
    } else {
		fputs(_("NAME? "), stderr);
